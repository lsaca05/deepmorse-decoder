...
class Model:
    ...
    def __init__(
        ...
        if TF2 is True:
            ...
            self.inputImgs = tf.keras.Input(
                dtype=tf.float32, shape=(self.imgSize[0], self.imgSize[1])
            )
        else:
        ...
        if TF2 is True:
            self.learningRate = tf.squeeze(
                tf.keras.Input(dtype=tf.float32, shape=[], batch_size=1)
            )
        else:
    ...
    def setupRNN(self):
        ...
        if TF2 is True:
            cells = [
                tf.keras.layers.LSTMCell(units=numHidden) for _ in range(2)
            ]  # 2 layers
        else:
            cells = [
    ...
    def setupCTC(self):
        ...
        if TF2 is True:
            self.gtTexts = tf.SparseTensor(
                ...
                tf.keras.Input(dtype=tf.int32, shape=[]),
                tf.keras.Input(dtype=tf.int64, shape=[], batch_size=2),
            )
        else:
            self.gtTexts = tf.SparseTensor(
        ...
        # calc loss for batch
        if TF2 is True:
            # Tensor("input_3:0", shape=(None,), dtype=int32)
            self.seqLen = tf.keras.Input(dtype=tf.int32, shape=[])
        else:
            # Tensor("Placeholder_2:0", shape=(None,), dtype=int32)
        ...
        if TF2 is True:
            self.savedCtcInput = tf.keras.Input(
                dtype=tf.float32,
                shape=[None, len(self.charList) + 1],
                batch_size=self.maxTextLen,
            )
        else:
        ...
        if self.decoderType == DecoderType.BestPath:
            ...
            wordChars = (
                open("Volumes/Elements/" + self.modelDir + "wordCharList.txt")
                .read()
                .splitlines()[0]
            )
